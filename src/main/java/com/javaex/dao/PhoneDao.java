...
public class PhoneDao {
	...
	/*
	 * 
	 * // 0. import java.sql.*; private Connection conn = null; private
	 * PreparedStatement pstmt = null; private ResultSet rs = null;
	 * 
	 * 
	 * private void getConnection() { try { // 2. Connection 얻어오기 conn =
	 * dataSource.getConnection(); // System.out.println("접속성공"); } catch
	 * (SQLException e) { System.out.println("error:" + e); } }
	 * 
	 * public void close() { // 5. 자원정리 try { if (rs != null) { rs.close(); } if
	 * (pstmt != null) { pstmt.close(); } if (conn != null) { conn.close(); } }
	 * catch (SQLException e) { System.out.println("error:" + e); } }
	 * 
	 * // 사람 추가 public int personInsert(PersonVo personVo) { int count = 0;
	 * getConnection(); sqlSession. try {
	 * 
	 ...
	 * getPersonList(""); }
	 * 
	 * // 사람 리스트(검색할때) public List<PersonVo> getPersonList(String keword) {
	 ...
	 * 
	 * if (keword != "" || keword == null) { query += " where name like ? "; query
	 ...
	 * pstmt.setString(3, '%' + keword + '%'); // ?(물음표) 중 3번째, 순서중요 } else { pstmt
	 * = conn.prepareStatement(query); // 쿼리로 만들기 }
	 * 
